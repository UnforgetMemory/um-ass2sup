...
- **`ass2sup-cli`: Progress bar invisible on Windows PowerShell** — the indicatif progress bar drew to stdout, where the tracing stdout layer contended with it, so on Windows the bar was swallowed entirely and long renders looked "frozen". The bar now draws to **stderr** with a 100 ms steady tick, so it renders reliably in both backends.
