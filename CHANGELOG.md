...
- **`subtitle-renderer-libass`: Non-recursive font scan** — `FontCache::scan_fonts` only scanned the top level of each font directory, missing every font nested in subdirectories (`/usr/share/fonts/truetype/dejavu/`, `~/.local/share/fonts/truetype/MiSans/`, etc.) — on typical Linux setups this meant 0–4 fonts found. Now walks directories recursively; a test asserts nested TTF/OTF discovery.
- **`subtitle-renderer-libass`: Filtered font registration could register 0 fonts** — when the needed-families filename filter matched nothing (Windows ships `msyh.ttc` for family "Microsoft YaHei", so stem matching fails), zero fonts were registered and every frame rendered empty. Now falls back to a full scan/cache when the filtered result is empty.
- **`ass2sup-cli`: Silent 0-frame success** — a libass render that produced 0 frames previously wrote an empty SUP and reported success. Now both `libass::render_and_quantize` and `convert_file` return an explicit error explaining the likely cause (fonts missing / no visible dialogue).
- **`ass2sup-cli`: Progress bar invisible on Windows PowerShell** — the indicatif progress bar drew to stdout, where the tracing stdout layer contended with it, so on Windows the bar was swallowed entirely and long renders looked "frozen". The bar now draws to **stderr** with a 100 ms steady tick.
- **`ass2sup-cli`: No progress feedback without a TTY** — indicatif still relies on terminal ANSI redraws, which Windows PowerShell can swallow even on stderr. Added a `ProgressReporter` that additionally emits a throttled plain-text log (`Rendered X/Y (P%) elapsed Es, ETA ~Es`) every 3 s or 500 frames through the stdout tracing layer, so progress is visible on every platform and in redirected output. Used by both the native and libass render loops.
- **`ass2sup-cli`: No stage/timing feedback** — long native renders gave no sense of progress or remaining time. Each pipeline phase now logs elapsed time: `Prepared N render timestamps (video ~Xs) in Ys`, `render+quantize done in Xs (avg Y ms/frame)`, `Encoded N segments → M bytes in Xs`.
- **`ass2sup-cli`: No file logging** — added `--log-file <PATH>` to write timestamped diagnostic logs (INFO+ with target/file/line) to a file in addition to the console.
