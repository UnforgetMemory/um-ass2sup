...
pub fn parse_effect(s: &str) -> Effect {
    ...
    match keyword {
        ...
        kw if kw.eq_ignore_ascii_case("Scroll up") => {
            let delay_per_row = parts
                .get(1)
                .and_then(|v| v.trim().parse().ok())
                .unwrap_or(0);
            let top_offset = parts
                .get(2)
                .and_then(|v| v.trim().parse().ok())
                .unwrap_or(0.0);
            let bottom_offset = parts
                .get(3)
                .and_then(|v| v.trim().parse().ok())
                .unwrap_or(0.0);
            Effect::ScrollUp {
