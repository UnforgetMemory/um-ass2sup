...
use cosmic_text::{CacheKey, CacheKeyFlags, SwashImage, Weight};
use fontdb;
...
/// Cosmic-text rendering resources: font system, swash cache, pixmap pool.
pub struct CosmicRenderResources {
    ...
    cjk_scan_cache: Mutex<Option<Option<fontdb::ID>>>,
    /// Per-font CJK capability: `true` = face has U+4E2D glyph.
    cjk_glyphs_cache: Mutex<HashMap<fontdb::ID, bool>>,
...
impl CosmicRenderResources {
    ...
    pub fn load_font_file(&self, path: &Path) -> Result<fontdb::ID, String> {
        let mut fs = self.font_system.lock();
        let db = fs.db_mut();
    ...
    ///
    /// Returns the font ID of the last face, or `0` if parsing failed.
    pub fn load_font_data(&self, data: Vec<u8>) -> fontdb::ID {
        let mut fs = self.font_system.lock();
        let db = fs.db_mut();
        db.load_font_data(data);
        let id = db.faces().last().map(|f| f.id).unwrap_or_default();
        self.cjk_scan_cache.lock().take();
        self.cjk_glyphs_cache.lock().clear();
        id
    }

    /// Resolves a font family name to a font ID using fontdb query.
    pub fn resolve_font(&self, family: &str, bold: bool, italic: bool) -> Option<fontdb::ID> {
    ...
    pub fn query_cjk_capable_any(&self) -> Option<fontdb::ID> {
        ...
        let face_ids: Vec<fontdb::ID> = fs.db().faces().map(|f| f.id).collect();
        ...
        let result = fs
            ...
            .with_face_data(id, |data, index| {
                swash::FontRef::from_index(data, index as usize)
                    .is_some_and(|font| font.charmap().map('\u{4E2D}') != 0)
            })
            .unwrap_or(false);
        drop(fs);
    ...
    pub fn font_system(&self) -> parking_lot::MutexGuard<'_, cosmic_text::FontSystem> {
        self.font_system.lock()
    }

    /// Provides mutable access to the underlying SwashCache for rasterization.
    pub fn swash_cache(&self) -> parking_lot::MutexGuard<'_, cosmic_text::SwashCache> {
        self.swash_cache.lock()
    }

    /// Returns the inner FontSystem mutex for more complex locking patterns.
    pub fn font_system_mutex(&self) -> &Mutex<cosmic_text::FontSystem> {
        &self.font_system
    }

    /// Returns the inner SwashCache mutex for more complex locking patterns.
    pub fn swash_cache_mutex(&self) -> &Mutex<cosmic_text::SwashCache> {
        &self.swash_cache
    }
}

impl Default for CosmicRenderResources {
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn render_event_cosmic(
    ...
    let oxf = ox as f32;
    let oyf = oy as f32;
    let fs = &mut cosmic.font_system.lock();
    let cache = &mut cosmic.swash_cache.lock();
