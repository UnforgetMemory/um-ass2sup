fn main() {
    let sup_path = std::env::args().nth(1).expect("Usage: decode_sup_to_png <input.sup> <output_dir>");
    let out_dir = std::env::args().nth(2).expect("Usage: decode_sup_to_png <input.sup> <output_dir>");
    ...
    for (i, ds) in display_sets.iter().enumerate() {
        match pgs_encoder::decode_frame_to_rgba(ds, &mut ctx, 255) {
            Ok(frame) => {
                ...
                println!("frame {:03}: {}x{} ({} bytes) → {}", i, frame.width, frame.height, png_bytes.len(), path);
            }
            Err(e) => {
